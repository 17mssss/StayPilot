...
export default function Register() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault()
    setError(null)
    if (!form.firstName.trim() || !form.lastName.trim()) { setError('Veuillez renseigner votre prénom et nom.'); return }
    if (form.password !== form.confirm) { setError('Les mots de passe ne correspondent pas.'); return }
    if (strength < 3) { setError('Votre mot de passe est trop faible.'); return }
    ...
    if (err) {
      setError(err.toLowerCase().includes('already') ? 'Un compte existe déjà avec cet email.' : err)
    } else {
      setSuccess(true)
    ...
    <div className="min-h-screen bg-bg flex items-center justify-center px-4 py-8 transition-colors">
      <div className="w-full max-w-md">
        <div className="flex flex-col items-center mb-6">
          ...
          <p className="text-sm text-muted">Interface d'administration</p>
        </div>

        <div className="bg-surface rounded-2xl shadow-sm border border-border p-5 sm:p-8 transition-colors">
          <h2 className="text-xl font-semibold text-dark mb-6">Créer un compte</h2>
          ...
          {success ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input value={form.email} onChange={set('email')} type="email" placeholder="admin@staypilot.fr" required
                  className="w-full px-3 py-3 text-base sm:text-sm border border-border rounded-lg focus:outline-none focus:border-primary bg-bg text-dark placeholder:text-muted transition-colors" />
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input value={form.password} onChange={set('password')} type={showPassword ? 'text' : 'password'} placeholder="••••••••" required
                    className="w-full px-3 py-2.5 pr-10 text-sm border border-border rounded-lg focus:outline-none focus:border-primary bg-bg text-dark placeholder:text-muted transition-colors" />
                  ...
                  <div className="mt-2">
                    ...
                    <ul className="mt-1.5 space-y-0.5">
                      {[
                        ...
                        <li key={label} className={`text-xs flex items-center gap-1.5 ${ok ? 'text-green-600' : 'text-muted'}`}>
                          <CheckCircle size={11} className={ok ? 'text-green-500' : 'text-muted'} />
                          {label}
                        </li>
                      ))}
                    </ul>
              ...
              <div>
                ...
                <div className="relative">
                  <input value={form.confirm} onChange={set('confirm')} type={showConfirm ? 'text' : 'password'} placeholder="••••••••" required
                    className="w-full px-3 py-2.5 pr-10 text-sm border border-border rounded-lg focus:outline-none focus:border-primary bg-bg text-dark placeholder:text-muted transition-colors" />
